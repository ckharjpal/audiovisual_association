...
class GPLDA:
    ...
    def train_ml(self, data, spk_labs, pldaFilename=""):
        ...
        self.Sb = self.St - Sw
        if pldaFilename:
            print('\nSaving PLDA to file {}'.format(pldaFilename))
            h5write(pldaFilename, [self.St, self.Sb], ['St', 'Sb'])        

    def train_em(self, data, spk_labs, pldaFilename=""):
    ...
    def load(self, pldaFilename):
        self.St, self.Sb = h5read(pldaFilename, ['St', 'Sb'])
...
def wccn(data, labels, adapt_coef=0, adapt_ind_end=0):
    nclasses = np.unique(labels).size
    Sw = compute_within_cov(data, labels, nclasses, adapt_coef, adapt_ind_end)
...
def compute_within_cov(data, labels, nclasses, adapt_coef=0, adapt_ind_end=0):
    mu_c = compute_class_avg(data, labels, nclasses)  # numeric labels are assumed
    data_mu = data - mu_c[labels].T
    if adapt_coef>0 and adapt_ind_end==0:
        raise ValueError('Adapation stop index should be greater than 0!')
    Sw = (1 - adapt_coef) * np.cov(data_mu[:,adapt_ind_end:])
    if adapt_coef>0 and adapt_ind_end>0:
        Sw += adapt_coef * np.cov(data_mu[:,:adapt_ind_end])
    # Sw = data_mu.dot(data_mu.T)
    return Sw


def lda(data, labels, adapt_coef=0, adapt_ind_end=0):
    ...
    if adapt_coef>0 and adapt_ind_end==0:
        raise ValueError('Adapation stop index should be greater than 0!')
    St = (1 - adapt_coef) * np.cov(data[:, adapt_ind_end:]) 
    if adapt_coef>0 and adapt_ind_end>0:
        St += adapt_coef * np.cov(data[:, :adapt_ind_end])
    Sb = St - Sw
    D, V = eigh(Sb, Sw)
    D = D[::-1]
    V = V[:, ::-1]
