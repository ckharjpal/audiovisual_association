...
function gen_sad_marks(list, outdir, num_workers)
...
if nargin<3, num_workers = feature('numcores') - 2; end
...
for batch = 1 : nbatch
    ...
    parfor ix = 1 : length(infilenames_b)
        [pathstr,basename,~] = fileparts(infilenames_b{ix});
        sadFilename = fullfile(outdir, [pathstr,'/',basename, '.txt']);
        % here, we assume all audio files are single channel, otherwise you
        % should also specify the side (i.e., 'a' or 'b')
        [~, ~, speech_endpts, ~] = nist_segmenter(infilenames_b{ix}, 'a');
